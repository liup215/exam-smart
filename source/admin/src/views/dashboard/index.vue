...
import { index as dashboardIndex } from '@/api/dashboard'
export default {
  mixins: [resize],
  components: {
    CountTo
  ...
  mounted () {
    ...
    dashboardIndex().then(re => {
      ...
      _this.echartsUserAction.setOption(this.option('用户活跃度', '{b}日{c}度', response.monthDayText, response.monthDayUserActionValue))
      _this.echartsQuestion.setOption(this.option('题目月数量', '{b}日{c}题', response.monthDayText, response.monthDayDoExamQuestionValue))
      this.loading = false
    })
  },
  methods: {
